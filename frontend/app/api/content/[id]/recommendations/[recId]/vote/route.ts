...
export async function POST(
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${MOTIA_BACKEND_URL}/content/${id}/recommendations/${recId}/vote`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: typeof data === 'string' ? data : data.error || 'Failed to vote' },
    ...
    return NextResponse.json(
      { error: 'Failed to vote', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
