...
export async function GET(request: NextRequest) {
  try {
    // Derive ID from the URL path to avoid any issues with params
    const pathname = new URL(request.url).pathname;
    // Expected pattern: /api/content/:id
    ...
    const response = await fetchWithTimeout(`${MOTIA_BACKEND_URL}/content/${id}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: typeof data === 'string' ? data : data.error || 'Content not found' },
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: typeof data === 'string' ? data : data.error || 'Failed to delete content' },
        { status: response.status }
      );
    }
